...
#inf_steps = 120000/N # Each one is 2 epochs
inf_steps = 600000/N # Each one is 10 epochs
width = set_width
...
with tf.name_scope("posterior"):
	qW_fc1 = Normal(loc=tf.Variable(tf.random_normal([D, width])),
              scale=0.005*tf.nn.softplus(tf.Variable(tf.random_normal([D, width])))) 
	qb_fc1 = Normal(loc=tf.Variable(tf.random_normal([width])),
              scale=0.01*tf.nn.softplus(tf.Variable(tf.random_normal([width]))))

	# Contruct the q(w) and q(b). in this case we assume Normal distributions.
	qW_fc2 = Normal(loc=tf.Variable(tf.random_normal([width, K])),
              scale=0.1*tf.nn.softplus(tf.Variable(tf.random_normal([width, K])))) 
	qb_fc2 = Normal(loc=tf.Variable(tf.random_normal([K])),
              scale=0.5*tf.nn.softplus(tf.Variable(tf.random_normal([K]))))
        """
	qW_fc1 = Normal(loc=tf.Variable(tf.random_normal([D, width])),
	...
	qb_fc2 = Normal(loc=tf.Variable(tf.random_normal([K])),
        ...
        """
	qW_fc1 = Normal(loc=tf.Variable(tf.random_normal([D, width])),
              scale=0.5*tf.Variable(tf.random_normal([D, width]))) 
	qb_fc1 = Normal(loc=tf.Variable(tf.random_normal([width])),
              scale=0.5*tf.Variable(tf.random_normal([width])))

	# Contruct the q(w) and q(b). in this case we assume Normal distributions.
	qW_fc2 = Normal(loc=tf.Variable(tf.random_normal([width, K])),
              scale=0.5*tf.Variable(tf.random_normal([width, K]))) 
	qb_fc2 = Normal(loc=tf.Variable(tf.random_normal([K])),
              scale=0.5*tf.Variable(tf.random_normal([K])))
       """
...
inference = ed.KLqp({W_fc1: qW_fc1, b_fc1: qb_fc1, W_fc2: qW_fc2, b_fc2: qb_fc2 }, data={y: y_})
...
def save_weights_FAST(epochs_tn):
	import os
	if not os.path.exists("models%s/SampledModels_d=1_w=_%s_e=%s_b=%s_a=%s"%(conc, set_width, epochs_tn, 
											batch, activ)):
    		os.makedirs("models%s/SampledModels_d=1_w=_%s_e=%s_b=%s_a=%s"%(conc, set_width, epochs_tn, 
                                                                                        batch, activ))
	from tqdm import trange
	...
	for _ in trange(1000):
    		np.savez_compressed("models%s/SampledModels_d=1_w=_%s_e=%s_b=%s_a=%s/sample_weights_%s"%(conc, set_width, epochs_tn, 
                                                                                        batch, activ, _),
						[sW_fc1[_], sb_fc1[_], 
						sW_fc2[_], sb_fc2[_]], 
                                                ['wfc1', 'bfc1', 'w', 'b'])
...
np.savez_compressed("MNIST_Networks/VIMODEL_MNIST_1_" + str(width) + '_' + activ + ".net", [mW_0, mb_0, mW_1, mb_1,
                                    dW_0, db_0, dW_1, db_1], 
                                   ['mw0', 'mb0', 'mw1', 'mb1',
                                    'dw0', 'db0', 'dw1', 'db1'])
# In[19]:
