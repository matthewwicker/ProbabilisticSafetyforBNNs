...
def gen_samples(iters):
    ...
    sW_2 = np.random.normal(mW_2, dW_2, (iters, mW_2.shape[0], mW_2.shape[1]))
    sb_2 = np.random.normal(mb_2, db_2, (iters, mb_2.shape[0]))
    GLOBAL_samples = [sW_0, sb_0, sW_1, sb_1, sW_2, sb_2]
...
def interval_bound_propagation_VCAS(a):
    ...
    [sW_0, sb_0, sW_1, sb_1, sW_2, sb_2] = GLOBAL_samples
    ...
    sW_2 = sW_2[id*search_samps:(id+1)*search_samps]
    sb_2 = sb_2[id*search_samps:(id+1)*search_samps]
    ...
    for i in range(search_samps): 
	    ...
	    continue
            ...
            h1 = my_relu(np.matmul(x, sW_0[i]) + sb_0[i])
            h2 = my_relu(np.matmul(h1, sW_1[i]) + sb_1[i])
            y = np.matmul(h2, sW_2[i]) +sb_2[i]
        ...
        if(np.argmax(y) == out_ind or extra_gate):
            ...
            h_l, h_u = propagate_interval(sW_1[i], dW_1, sb_1[i], db_1, h_l, h_u, w_margin)
            h_l, h_u = my_relu(h_l), my_relu(h_u)
            y_pred_l, y_pred_u = propagate_interval(sW_2[i], dW_2, sb_2[i], db_2, h_l, h_u, w_margin)
            #print "================================"
            #print y_pred_l
            #print "--------------------------------"
            #print y
...
def compute_all_intervals_proc(a):
    ...
    if(ind == 0):
        ...
        for i in range(len(vW_0)):
            args = []
            for j in range(len(vW_0[i])):
                args.append((vW_0[i][j], margin, mW_2[i][j], dW_2[i][j]))
            arr = p.map(compute_single_prob, args)
            pW_0[i] = arr
    elif(ind == 1):
        ...
        args = []
        for i in range(len(vW_0)):
            args.append((vW_0[i], margin, mb_2[i], db_2[i]))
        pW_0 = p.map(compute_single_prob, args)
